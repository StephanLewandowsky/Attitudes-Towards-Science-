...
source(paste(wd,"darwinFuncs.r",sep="/"))
darwin <- read.csv(paste(wd,"full1200downloadedAnonymized.csv",sep="/"),header=TRUE,row.names=NULL) 
...
ggsave("slidersSocDarw.pdf", plot=slidp, width=6.5, height=9)
...
write.table(t4l[1:5,],file="_t.freemarket.tex",quote=FALSE,col.names=FALSE,row.names=FALSE)
write.table(t4l[6:10,],file="_t.evo.tex",quote=FALSE,col.names=FALSE,row.names=FALSE)
write.table(t4l[11:15,],file="_t.cam.tex",quote=FALSE,col.names=FALSE,row.names=FALSE)
write.table(t4l[16:20,],file="_t.mwevo.tex",quote=FALSE,col.names=FALSE,row.names=FALSE)
write.table(t4l[21:25,],file="_t.mwnat.tex",quote=FALSE,col.names=FALSE,row.names=FALSE)
write.table(t4l[26:30,],file="_t.mwequ.tex",quote=FALSE,col.names=FALSE,row.names=FALSE)
write.table(t4l[31:35,],file="_t.relig.tex",quote=FALSE,col.names=FALSE,row.names=FALSE)
write.table(t4l[36:40,],file="_t.vax.tex",quote=FALSE,col.names=FALSE,row.names=FALSE)
...
#make copy first if desired: darwin1.5b <- darwin1.5  |  darwin1.5 <- darwin1.5b
...
pdf(file="histoSocDarw.pdf",height=9,width=6.5) 
par(mfrow=c(4,2))
clusterlabels <- c("Free market", "Evolution", "CAM", "Men/women evolved differently", "Men/women naturally different",
...
modelCorrel <- c("
 ...
 write.table(cormat[13:20],file="_t.lvcor.tex",quote=FALSE,col.names=FALSE,row.names=FALSE) 
...
#* ------------- Now model various scientific propositions  -----------------------------------------------
# Evolution : with mwnatFac as well as mwevoFac as predictor, cov matrix not positive definite.
 ...
 modelVax <- c("  vaxFac   ~ fmFac +  mwequFac + camFac + consFac 
                  mwevoFac ~~ 0*religFac
                  mwevoFac ~~ 0*vaxFac
                  mwevoFac ~~ 0*fmFac
                  consFac  ~~ 0*vaxFac
 ...
 # Vax & evolution: first full, then slightly constrained model (i.e., with covariances between factors set to 0) 
 modelVaxEvofull <- c("  vaxFac   ~ fmFac +  mwequFac + camFac + consFac 
 ...
 fitVaxEvofull <- sem(modelVaxEvofull, compositedarwin, std.lv=TRUE, estimator="ML")
 summary(fitVaxEvofull,standardized=TRUE, fit.measures=TRUE)
 
 
  modelVaxEvo <- c("  vaxFac   ~ fmFac +  mwequFac + camFac + consFac 
                      evoFac   ~ fmFac +  religFac + mwequFac + mwevoFac  + consFac 
 ...
 fitVaxEvo <- sem(modelVaxEvo, compositedarwin, std.lv=TRUE, estimator="ML")
 summary(fitVaxEvo,standardized=TRUE, fit.measures=TRUE)
 aovfullvfin <- anova(fitVaxEvofull,fitVaxEvo)
 aovfullvfindf<- aovfullvfin[["Df"]][2]- aovfullvfin[["Df"]][1]
 
 finalmodelgof <- fitMeasures(fitVaxEvo)
 modificationIndices(fitVaxEvo, sort. = TRUE, maximum.number = 4)
 lavInspect(fitVaxEvo, what = "cor.lv")
 eigen( inspect(fitVaxEvo, "cov.lv") )$values 
 #x11()
 pdf(file="finalSEM.pdf",height=8,width=8) 
 semPaths(fitVaxEvo, what="std", title =FALSE, curvePivot = TRUE,residuals=FALSE, 
          curvature=5, nCharNodes=0, edge.label.cex=1.2,mar=c(3,10,3,3),
          structural=TRUE, layout="tree",rotation=2)
 dev.off()
 
 # Vax & evolution without FM
 modelVaxEvo2 <- c("  vaxFac   ~ 0*fmFac +  mwequFac + camFac + consFac 
                  evoFac   ~ 0*fmFac +  religFac + mwequFac + mwevoFac  + consFac 
                  mwevoFac ~~ 0*consFac
                  camFac ~~ 0*mwequFac
                  camFac ~~ 0*mwevoFac
                  camFac ~~ 0*mwnatFac
 ...
 fitVaxEvo2 <- sem(modelVaxEvo2, compositedarwin, std.lv=TRUE, estimator="ML")
 summary(fitVaxEvo2,standardized=TRUE, fit.measures=TRUE)
 finalmodelgof2 <- fitMeasures(fitVaxEvo2)
 anova(fitVaxEvo2,fitVaxEvo)

 
 # Vax & evolution without cons
 modelVaxEvo3 <- c("  vaxFac   ~ fmFac +  mwequFac + camFac + 0*consFac 
                  evoFac   ~ fmFac +  religFac + mwequFac + mwevoFac  + 0*consFac 
                  mwevoFac ~~ 0*consFac
                  camFac ~~ 0*mwequFac
                   camFac ~~ 0*mwevoFac
                   camFac ~~ 0*mwnatFac
 ...
 fitVaxEvo3 <- sem(modelVaxEvo3, compositedarwin, std.lv=TRUE, estimator="ML")
 summary(fitVaxEvo3,standardized=TRUE, fit.measures=TRUE)
 finalmodelgof3 <- fitMeasures(fitVaxEvo3)
  anova(fitVaxEvo3,fitVaxEvo)
...
## ---- Create mean-indicators for all clusters for correlation with CRT and deal with lavaan warning ---
meanIndicators <- NULL
for (cluster in c("C","E","F","H","I","J","D","G")) {
...
write.table(cormat4p[13:20],file="_t.compcor.tex",quote=FALSE,col.names=FALSE,row.names=FALSE) 
write.table(cormat4p[21],file="_t.crtcor.tex",quote=FALSE,col.names=FALSE,row.names=FALSE) 
...
if (lowerab==0 & upperab==1) {
  ggfn <- "conslibevogrid.pdf"  
} else {
  ggfn <- paste("conslibevogrid",lowerab,upperab,".pdf",sep="")
}
...
if (lowerab==0 & upperab==1) {
    ggfn <- "conslibequalitygrid.pdf"  
} else {
    ggfn <- paste("conslibequalitygrid",lowerab,upperab,".pdf",sep="")
}
