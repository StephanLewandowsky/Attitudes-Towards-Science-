...
\Sexpr{ifelse(correctaffirmbias,"","UNCORRECTED ANALYSIS")}
...
strong predictor of science understanding \citeA{Shtulman14}, and we therefore
...
and \Sexpr{round(crtperc[4],1)}, respectively. (Missing values
...
\\
CRT &
\Sexpr{round(as.numeric(crtmodgof["chisq"]),2)} & 
\Sexpr{round(as.numeric(crtmodgof["df"]))} & 
\Sexpr{round(as.numeric(crtmodgof["srmr"]),3)} & 
\Sexpr{round(as.numeric(crtmodgof["cfi"]),3)} & 
\Sexpr{round(as.numeric(crtmodgof["rmsea"]),3)} &
\Sexpr{round(as.numeric(crtmodgof["rmsea.ci.lower"]),3)} -- 
\Sexpr{round(as.numeric(crtmodgof["rmsea.ci.upper"]),3)} & \multicolumn{1}{c}{N/A} \\
...
The results generally replicate relevant previous research. The modest
negative correlation between free-market endorsement and acceptance of
vaccinations parallels a similar association observed by \citeA{Hornsey18},
...
The positive correlation between free-market endorsement and conservatism
has been observed previously \cite{Lewandowsky13b}, as has been the 
strong association between religiosity and conservatism \cite{Malka12}.
Likewise, the strong
...
acceptance of vaccinations \cite<e.g.,>{Attwell18,Browne15,Bryden18,Ernst02}.
...
full model, $\chi^2$(\Sexpr{aovresult$"Df diff"[2]})$=$\Sexpr{round(aovresult$"Chisq diff"[2],2)}, 
$p < \Sexpr{sprintf("%5.4f",aovresult$"Pr(>Chisq)"[2]+.0001)}$.
The fully constrained model, by contrast, 
incurred a significant loss of fit compared to the partially constrained model, 
$\chi^2$(\Sexpr{aovresult$"Df diff"[3]})$=$\Sexpr{round(aovresult$"Chisq diff"[3],2)}, 
$p < \Sexpr{sprintf("%5.4f",aovresult$"Pr(>Chisq)"[3]+.0001)}$.
...
has been observed previously. For example, \citeA{Lewandowsky13b} found that conservatism
...
SRMR=
\Sexpr{round(as.numeric(finalpolmodelgof["srmr"]),3)}; 
CFI=\Sexpr{round(as.numeric(finalpolmodelgof["cfi"]),3)}; RMSEA= 
\Sexpr{round(as.numeric(finalpolmodelgof["rmsea"]),3)}, CI:
\Sexpr{round(as.numeric(finalpolmodelgof["rmsea.ci.lower"]),3)} -- 
\Sexpr{round(as.numeric(finalpolmodelgof["rmsea.ci.upper"]),3)},
 and is shown in Figure~\ref{fig:finalpolSEM}.
\begin{figure}[tp] %fig:finalpolSEM
	\fitfigure{finalpolSEM.pdf}
...
fit acceptably well, 
$\chi^2(\Sexpr{supermodelgof["df"]})=\Sexpr{round(supermodelgof["chisq"],2)}$; 
SRMR=\Sexpr{round(supermodelgof["srmr"],3)}; CFI=\Sexpr{round(supermodelgof["cfi"],3)}; 
RMSEA=\Sexpr{round(supermodelgof["rmsea"],3)} (90\% CI:
\Sexpr{round(supermodelgof["rmsea.ci.lower"],3)}--
\Sexpr{round(supermodelgof["rmsea.ci.upper"],3)}).
\begin{figure}[tp] %fig:superSEM
\fitfigure{superSEM.pdf}
...
In our predictive SEM model (Figure~\ref{fig:superSEM}), we found that 
...
attitudes in our predictive model (Figure~\ref{fig:superSEM}),
...
predictive model; Figure~\ref{fig:superSEM}). 
...
We now turn to the main focus of our study,
