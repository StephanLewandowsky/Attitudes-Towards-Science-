The shortest addition to clarify would be to call it the original item list (or unselected/unfiltered list), and add a brief reference to the paper when discussing the fitting problems.
While the two items seemed to have added some noise, I am not sure that they actually reduce the quality of the measures (and starting to test all combinations seems to be questionable). Also, the inter-correlations for welfare benefits do not seem to be strikingly higher than those for immigration… 
...
knitr::opts_chunk$set(results="hide",echo=FALSE,message=FALSE,error=FALSE,cache=TRUE)
@
...
\begin{table} %tab:descriptives
	...
	\begin{tabular}{l r r c }
		...
		Socio-political conservatism &
		\Sexpr{round(consSI$SDSImod+.0001,2)} &  %to ensure printing of zero?
...
$\chi^2(\Sexpr{gofpolmodelgender["df"]})=\Sexpr{round(gofpolmodelgender["chisq"],2)}$; 
SRMR=\Sexpr{round(gofpolmodelgender["srmr"],3)}; CFI=\Sexpr{round(gofpolmodelgender["cfi"],3)}; 
RMSEA=\Sexpr{round(gofpolmodelgender["rmsea"],3)} (90\% CI:
\Sexpr{round(gofpolmodelgender["rmsea.ci.lower"],3)}--
\Sexpr{round(gofpolmodelgender["rmsea.ci.upper"],3)}).
